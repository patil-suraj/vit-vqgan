...
class Transformer(nn.Module):
    ...
    def setup(self):
        self.layers = [
            TransformerBlock(self.config, name=str(i), dtype=self.dtype)
...
class VitDecoder(nn.Module):
    ...
    def setup(self):
        ...
        self.position_embeddings = nn.Embed(self.num_patches, self.config.hidden_size, embedding_init=jax.nn.initializers.normal())
        self.transformer = Transformer(self.config, dtype=self.dtype)
        self.feed_forward_layer = FeedForwardLayer(self.config, dtype=self.dtype)
...
class VitVQModule(nn.Module):
    ...
    def setup(self):
        ...
        self.factor_in = nn.Dense(self.config.embed_dim, dtype=self.dtype, kernel_init=jax.nn.initializers.normal(0.01))
        self.quantizer = VectorQuantizer(self.config, dtype=self.dtype)
        self.factor_out = nn.Dense(self.config.hidden_size, dtype=self.dtype, kernel_init=jax.nn.initializers.normal(0.01))
